...
def test_tokenToTokenLimitOrder(auto, uni_router2, any, dai, uniLS, input_amount, whale_amount):
    ...
    call_data = uniLS.tokenToTokenRange.encode_input(auto.CHARLIE, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, input_amount, limit_output, int(MAX_UINT/input_amount), path, auto.CHARLIE, time.time() * 2)
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uf.address:
            with reverts(REV_MSG_USERFORW):
                uniLS.tokenToTokenRange(auto.CHARLIE, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, 1, 1, BIG_NUM, [], auto.CHARLIE, time.time() * 2, {'from': addr})
...
def test_tokenToTokenLimitOrder_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.tokenToTokenRange(auto.CHARLIE, max_gas_price, UNIV2_ROUTER2_ADDR, 1, 1, BIG_NUM, [], auto.CHARLIE, time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})