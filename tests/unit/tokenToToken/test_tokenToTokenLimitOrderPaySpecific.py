...
def test_tokenToTokenLimitOrderPaySpecific_eth(auto, evmMaths, uni_router2, any, dai, uniLS):
    ...
    call_data = uniLS.tokenToTokenLimitOrderPaySpecific.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, default_fee_info, input_amount, limit_output, path, time.time() * 2)
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uff.address:
            with reverts(REV_MSG_USERFEEFORW):
                uniLS.tokenToTokenLimitOrderPaySpecific(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, E_18, 1, [], time.time() * 2, {'from': addr})
...
def test_tokenToTokenLimitOrderPaySpecific_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.tokenToTokenLimitOrderPaySpecific(auto.CHARLIE, MIN_GAS, max_gas_price, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, 1, 1, [], time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})