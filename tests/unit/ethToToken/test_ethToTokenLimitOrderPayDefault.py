...
def test_ethToTokenLimitOrderPayDefault_eth(auto, evmMaths, uni_router2, any, uniLS):
    ...
    call_data = uniLS.ethToTokenRangePayDefault.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, limit_output, int(MAX_UINT/input_amount), path, time.time() * 2)
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uff.address:
            with reverts(REV_MSG_USERFEEFORW):
                uniLS.ethToTokenRangePayDefault(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, 1, MAX_UINT, [], time.time() * 2, {'from': addr})
...
def test_ethToTokenLimitOrderPayDefault_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.ethToTokenRangePayDefault(auto.CHARLIE, MIN_GAS, max_gas_price, UNIV2_ROUTER2_ADDR, 1, MAX_UINT, [], time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})