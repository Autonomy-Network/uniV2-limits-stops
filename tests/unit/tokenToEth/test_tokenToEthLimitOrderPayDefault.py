...
def test_tokenToEthLimitOrderPayDefault_eth(auto, evmMaths, uni_router2, any, uniLS):
    ...
    call_data = uniLS.tokenToEthRangePayDefault.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, input_amount, limit_output, int(MAX_UINT/input_amount), path, time.time() * 2)
...
def test_tokenToEthLimitOrderPayDefault_random(auto, evmMaths, uni_router2, any, uniLS, input_token_is_AUTO, input_amount, whale_amount, expected_gas, pay_with_AUTO):
    ...
    init_output = uni_router2.getAmountsOut(input_amount, path)[-1]
    limit_output = int(init_output * 1.1)
    call_data = uniLS.tokenToEthRangePayDefault.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, input_amount, limit_output, int(BIG_NUM/input_amount), path, time.time() * 2)
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uff.address:
            with reverts(REV_MSG_USERFEEFORW):
                uniLS.tokenToEthRangePayDefault(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, E_18, 1, E_18, [], time.time() * 2, {'from': addr})
...
def test_tokenToEthLimitOrderPayDefault_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.tokenToEthRangePayDefault(auto.CHARLIE, MIN_GAS, max_gas_price, UNIV2_ROUTER2_ADDR, 1, 1, E_18, [], time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})