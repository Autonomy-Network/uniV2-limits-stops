...
def test_tokenToEthLimitOrderPaySpecific_eth(auto, evmMaths, uni_router2, any, uniLS):
    ...
    call_data = uniLS.tokenToEthRangePaySpecific.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, input_amount, limit_output, int(MAX_UINT/input_amount), path, time.time() * 2)
...
def test_tokenToEthLimitOrderPaySpecific_AUTO(auto, evmMaths, uni_router2, any, uniLS):
...
def test_tokenToEthLimitOrderPaySpecific_AUTO_trade_AUTO(auto, evmMaths, uni_router2, any, uniLS):
...
def test_tokenToEthLimitOrderPaySpecific_random(auto, evmMaths, uni_router2, any, uniLS, input_token_is_AUTO, input_amount, whale_amount, expected_gas, pay_with_AUTO):
    if pay_with_AUTO:
        default_fee_info = (UNIV2_ROUTER2_ADDR, (ADDR_0, auto.AUTO) if input_token_is_AUTO else (any, WETH_ADDR, auto.AUTO), True)
    else:
    ...
    init_output = uni_router2.getAmountsOut(input_amount, path)[-1]
    limit_output = int(init_output * 1.1)
    call_data = uniLS.tokenToEthRangePaySpecific.encode_input(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, default_fee_info, input_amount, limit_output, int(BIG_NUM/input_amount), path, time.time() * 2)
...
def test_tokenToEthLimitOrderPaySpecific_rev_sender(a, auto, uniLS):
    for addr in list(a) + auto.all:
        if addr.address != auto.uff.address:
            with reverts(REV_MSG_USERFEEFORW):
                uniLS.tokenToEthRangePaySpecific(auto.CHARLIE, MIN_GAS, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, E_18, 1, E_18, [], time.time() * 2, {'from': addr})
...
def test_tokenToEthLimitOrderPaySpecific_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.tokenToEthRangePaySpecific(auto.CHARLIE, MIN_GAS, max_gas_price, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, 1, 1, E_18, [], time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})