...
def test_tokenToEthLimitOrderPaySpecific_eth(auto, evmMaths, uni_router2, any, uniLS):
    ...
    call_data = uniLS.tokenToEthLimitOrderPaySpecific.encode_input(auto.CHARLIE, MIN_GAS, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, input_amount, limit_output, path, time.time() * 2)
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uff.address:
            with reverts(REV_MSG_USERFEEFORW):
                uniLS.tokenToEthLimitOrderPaySpecific(auto.CHARLIE, MIN_GAS, UNIV2_ROUTER2_ADDR, DEFAULT_FEE_INFO, E_18, 1, [], time.time() * 2, {'from': addr})