...
def test_tokenToEthLimitOrder(auto, uni_router2, any, uniLS, input_amount, whale_amount):
    ...
    if uni_router2.getAmountsOut(input_amount, path)[-1] >= limit_output:
        ...
        assert auto.CHARLIE.balance() >= eth_start_bal - msg_value - req_eth_cost + limit_output
    ...
    for addr in list(a) + auto.all:
        if addr.address != auto.uf.address:
            with reverts(REV_MSG_USERFORW):
                uniLS.tokenToEthRange(auto.CHARLIE, MAX_GAS_PRICE, UNIV2_ROUTER2_ADDR, 1, 1, [], auto.CHARLIE, time.time() * 2, {'from': addr})
...
def test_tokenToEthLimitOrder_rev_gasPrice(a, auto, uniLS, max_gas_price, gas_price):
    if gas_price > max_gas_price:
        with reverts(REV_MSG_GASPRICE_HIGH):
            uniLS.tokenToEthRange(auto.CHARLIE, max_gas_price, UNIV2_ROUTER2_ADDR, 1, 1, [], auto.CHARLIE, time.time() * 2, {'from': auto.DEPLOYER, 'gasPrice': gas_price})