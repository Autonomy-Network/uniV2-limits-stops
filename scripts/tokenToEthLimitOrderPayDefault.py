...
def main():
    ...
    path = [token, WETH_ADDR]
    # Using USDT which has 6 decimal places, annoyingly
    input_amount = int(0.1 * 10**6)
    init_output = uni.getAmountsOut(input_amount, path)[-1]
    call_data = uniLS.tokenToEthRangePayDefault.encode_input(SENDER, MIN_GAS, 10**21, UNIV2_ADDR, input_amount, init_output, MAX_UINT, path, time.time() * 2)
